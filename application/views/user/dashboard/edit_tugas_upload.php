...
<div class="container">
	<div class="row">
		<div class="col" style="text-align: center; width: 80%; margin: auto;">
			<form action="<?= base_url("index.php/control_upload/upload_tugas/") ?><?= $enrol."/".$post_id; ?>" id="my-dropzone" method="post" class="dropzone" enctype="multipart/form-data">
		  		<div class="fallback">
...
<script type="text/javascript">
		...
		var myDropzone = new Dropzone("#my-dropzone", {
			...
			removedfile: function(file) {
				...
				$.ajax({
					type: "post",
					url: "<?= base_url("index.php/control_upload/remove_tugas/")?><?= $enrol."/".$post_id; ?>",
					data: { file: name },
					dataType: 'html'
				});
			...
			init: function() {
				var me = this;
				$.get("<?= base_url("index.php/control_upload/list_files_tugas/") ?><?= $enrol."/".$post_id; ?>", function(data) {
					// if any files already in server show all here
					if (data.length > 0) {
						$.each(data, function(key, value) {
							var mockFile = value;
							me.emit("addedfile", mockFile);
							me.emit("thumbnail", mockFile, "<?php echo base_url('uploads/upload_mhs/'); ?><?= $enrol."/".$post_id; ?>/" + value.name);
